...
import { WomanTest as Test } from "@/app/types/tests";
...
export default function WomanTest() {
  ...
  const handleNextCheckAge = (variant: string,) => {
    ...
    if (variant !== 'a' && currentQuestionId === '5') {
      setCurrentQuestionId('6');
    } else if (currentQuestionId === '6' && (variant !== 'a' && variant !== 'b')) {
      setCurrentQuestionId('7');
    } else if (currentQuestionId === '7' && (variant !== 'a' && variant !== 'b' && variant !== 'c')) {
      setCurrentQuestionId('8');
    }

    if (variant === 'a') {
      setCurrentQuestionId('9');
    } else if (currentQuestionId === '6' && variant === 'b') {
      setCurrentQuestionId('9');
    } else if (currentQuestionId === '7' && variant === 'c') {
      setCurrentQuestionId('9');
    }
  };
        ...
        {question ? (
              ...
              <Button
                ...
                onClick={() => {
                  if (currentQuestionId === '2') {
                    handleQuestionThree();
                  } else if (currentQuestionId === '5' || currentQuestionId === '6' || currentQuestionId === '7') {
          ...
          currentQuestionId === '9' && (
            ...
            <div className="flex flex-col gap-y-4">
              ...
              <div className="flex items-center justify-end">
                <WomanTestPrintPage ref={contentRef} answer={answers} />
                <Button
                  className="bg-[#1D7CBC] hover:bg-[#1D7CBC]/[0.8] border-none"
                  onClick={() => {
                    console.log(answers);
            ...
            <div className="flex flex-col gap-y-4">
              <h4 className="text-2xl font-medium">
                Внимание к себе — залог здоровья
              </h4>
              <p>
                В потоке дел здоровье часто отходит на второй план. «Женский чекап» создан, чтобы помочь вам:  
              </p>
              <ul className="pl-6 list-disc">
                <li>восстановить баланс,</li>
                <li>поддерживать энергию и здоровье,</li>
                <li>избежать ненужных рисков.</li>
              </ul>
              <p>Пройдите анкету и получите персональные рекомендации за 5 минут. <br /> Простая забота о себе за символическую стоимость. Начните сегодня!  </p>
              <div className="flex items-center justify-end">
                <Button
                  className="bg-[#1D7CBC] hover:bg-[#1D7CBC]/[0.8] border-none"
                  onClick={() => {
